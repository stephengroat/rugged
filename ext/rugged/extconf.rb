...
abort 'ERROR: GNU make is required to build Rugged.' unless MAKE
...
if arg_config('--use-system-libraries', !!ENV['RUGGED_USE_SYSTEM_LIBRARIES'])
  ...
  dir_config('git2').any? || pkg_config('libgit2')
  ...
  try_compile(<<-SRC) || abort("libgit2 version is not compatible, expected ~> #{major}.#{minor}.0")
...
#error libgit2 version is not compatible
  ...
  abort 'ERROR: CMake is required to build Rugged.' unless find_executable('cmake')
