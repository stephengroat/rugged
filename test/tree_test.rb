...
class TreeTest < Rugged::TestCase
  ...
  def test_tree_iteration
    enum_test = @tree.sort_by { |a| a[:oid] }.map { |e| e[:name] }.join(':')
  ...
  def test_tree_walk_only_trees
    @tree.walk_trees { |_root, entry| assert_equal :tree, entry[:type] }
  end

  def test_tree_walk_only_blobs
    @tree.walk_blobs { |_root, entry| assert_equal :blob, entry[:type] }
  end
...
class TreeUpdateTest < Rugged::TestCase
  ...
  def test_treebuilder_add
    builder = Rugged::Tree::Builder.new(@repo, @repo.head.target.tree)
    builder << { type: :blob, name: 'another-readme', oid: '1385f264afb75a56a5bec74243be9b367ba4ca08', filemode: 0o100644 }
    newtree = builder.write
    assert_equal '71a3bbe701e60c1756edd23cfc0b207711dca1f2', newtree
  end

  def test_tree_updater_add
    updates = [{ action: :upsert, path: 'another-readme', oid: '1385f264afb75a56a5bec74243be9b367ba4ca08', filemode: 0o100644 }]
  ...
  def test_treebuilder_add_nonexistent_fails
    ...
    assert_raises Rugged::TreeError do
      builder << { type: :blob, name: 'another-readme', oid: '0000000000000000000000000000000000000001', filemode: 0o100644 }
    end
  end

  def test_treebuilder_add_submodules_always_succeeds
    builder = Rugged::Tree::Builder.new(@repo, @repo.head.target.tree)
    builder << { type: :commit, name: 'submodule', oid: '0000000000000000000000000000000000000001', filemode: 0o160000 }
    newtree = builder.write
