...
class WalkerTest < Rugged::TestCase
  ...
  def test_walk_revlist
    ...
    oids = data.sort_by(&:oid).map { |a| a.oid[0, 5] }.join('.')
  ...
  def test_walk_revlist_with_limit
    ...
    oids = data.map(&:oid)
    assert_equal %w[9fd738e8f7967c078dceed8190330fc8648ee56a 4a202b346bb0fb0db7eff3cffeb3c70babbd2045], oids
  end
  ...
  def test_walk_revlist_with_offset
    ...
    oids = data.map(&:oid)
    assert_equal %w[5b5b025afb0b4c913b4c338a42934a3863bf3644 8496071c1b46c854b31185ea97743be6a8774479], oids
  end
  ...
  def test_walk_is_enumberable
    @walker.push('9fd738e8f7967c078dceed8190330fc8648ee56a')
    enum = @walker.sort_by(&:oid).map { |a| a.oid[0, 4] }.join('.')
