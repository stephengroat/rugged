...
class RepositoryTest < Rugged::TestCase
  ...
  def test_lookup_object
    ...
    assert object.is_a?(Rugged::Commit)
  end

  def test_find_reference
    ref = @repo.ref('refs/heads/master')

    assert ref.is_a?(Rugged::Reference)
  ...
  def test_expand_objects
    ...
    assert_equal expected, @repo.expand_oids(%w[a4a7dce8 a65fedf3 c47800c7 deadbeef])
  end

  def test_expand_and_filter_objects
    assert_equal 2, @repo.expand_oids(%w[a4a7dce8 1385f264af]).size
    assert_equal 1, @repo.expand_oids(%w[a4a7dce8 1385f264af], :commit).size
    assert_equal 2, @repo.expand_oids(%w[a4a7dce8 1385f264af], %w[commit blob]).size
    assert_equal 1, @repo.expand_oids(%w[a4a7dce8 1385f264af], [:commit, :tag]).size

    assert_raises RuntimeError do
      @repo.expand_oids(%w[a4a7dce8 1385f264af], [:commit, :tag, :commit]).size
    end

    assert_raises RuntimeError do
      @repo.expand_oids(%w[a4a7dce8 1385f264af], [:commit]).size
    end
  end
...
class RepositoryCloneTest < Rugged::TestCase
  ...
  def test_clone_with_transfer_progress_callback
    ...
    callsback = 0
    repo = Rugged::Repository.clone_at(@source_path, @tmppath, transfer_progress: lambda { |*args|
  ...
  def test_clone_with_update_tips_callback
    ...
    repo = Rugged::Repository.clone_at(@source_path, @tmppath, update_tips: lambda { |refname, a, b|
                                                                              calls += 1
  ...
  def test_clone_quits_on_error
    begin
      Rugged::Repository.clone_at(@source_path, @tmppath, transfer_progress: ->(*_) { raise 'boom' })
    rescue => e
  ...
  def test_clone_with_bad_progress_callback
    assert_raises ArgumentError do
      Rugged::Repository.clone_at(@source_path, @tmppath, transfer_progress: Object.new)
    end
    assert_no_dotgit_dir(@tmppath)
  end
