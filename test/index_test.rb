...
class IndexTest < Rugged::TestCase
  ...
  def test_get_entry_data
    ...
    assert_equal 1_273_360_380, e[:mtime].to_i
    assert_equal 1_273_360_380, e[:ctime].to_i
    assert_equal 4, e[:file_size]
    assert_equal 234_881_026, e[:dev]
    assert_equal 6_674_088, e[:ino]
    assert_equal 33_188, e[:mode]
  ...
  def test_iterate_entries
    itr_test = @index.sort_by { |a| a[:oid] }.map { |e| e[:path] }.join(':')
  ...
  def test_add_new_entries
    ...
    itr_test = @index.sort_by { |a| a[:oid] }.map { |x| x[:path] }.join(':')
...
class IndexWriteTest < Rugged::TestCase
  ...
  def test_can_write_index
    ...
    itr_test = index2.sort_by { |a| a[:oid] }.map { |x| x[:path] }.join(':')
...
class IndexAddAllTest < Rugged::TestCase
  ...
  def test_add_all_dry_run
    Dir.chdir(@repo.workdir) do
      yielded = []
      @repo.index.add_all do |path, _pathspec|
        yielded << path
        false
      end
