...
class OnlineCloneTest < Rugged::OnlineTestCase
  ...
  if Rugged.features.include?(:ssh) && ssh_creds?
    def test_clone_over_ssh_with_credentials
      Dir.mktmpdir do |dir|
        repo = Rugged::Repository.clone_at(ENV['GITTEST_REMOTE_SSH_URL'], dir, credentials: ssh_key_credential)
    ...
    def test_clone_over_ssh_with_credentials_from_agent
      Dir.mktmpdir do |dir|
        repo = Rugged::Repository.clone_at(ENV['GITTEST_REMOTE_SSH_URL'], dir, credentials: ssh_key_credential_from_agent)
    ...
    def test_clone_over_ssh_with_credentials_callback
      Dir.mktmpdir do |dir|
        repo = Rugged::Repository.clone_at(ENV['GITTEST_REMOTE_SSH_URL'], dir, credentials: lambda { |_url, _username, _allowed_types|
    ...
    def test_clone_callback_args_with_username
      Dir.mktmpdir do |dir|
        ...
        assert_raises Rugged::SshError do
          Rugged::Repository.clone_at('git@github.com:libgit2/TestGitRepository', dir, credentials: lambda { |*args|
