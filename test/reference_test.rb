...
class ReferenceWriteTest < Rugged::TestCase
  ...
  def test_create_unicode_reference_nfd
    ...
    expected_name = if @repo.config['core.precomposeunicode'] == 'true'
                      "refs/heads/\xC3\x85\x73\x74\x72\xC3\xB6\x6D"
                    else
                      ref_name
                    end
...
class ReflogTest < Rugged::TestCase
  ...
  def test_create_default_log_custom_log_message
    ref = @repo.references.create(
      ...
      'a65fedf39aefe402d3bb6e24df4d4f5fe4547750', message: 'reference created'
    )
    reflog = ref.log
  ...
  def test_set_target_default_log_custom_log_message
    @repo.references.update(@ref, '5b5b025afb0b4c913b4c338a42934a3863bf3644', message: 'reference updated')
